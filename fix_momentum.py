...
def repair_momentum_pwa():
    print("🚀 Executing Full Momentum PWA Recovery...")

    # ---------------------------------------------------------------------
    # 1. REPAIR SYNC.JS (Payload Sanitization & Capping max:20 fields)
    # ---------------------------------------------------------------------
    if os.path.exists('sync.js'):
        with open('sync.js', 'r', encoding='utf-8') as f:
            sync_code = f.read()

        clean_map_js = """function mapSessionToApiPayload(localSession) {
            ...
            for i in range(start_idx, len(sync_code)):
                if sync_code[i] == '{':
                    ...
                    if brace_count == 0:
                        ...
                        break
            if end_idx != -1:
                sync_code = sync_code[:start_idx] + clean_map_js + sync_code[end_idx:]
        else:
            sync_code += '\n' + clean_map_js + '\n'

        with open('sync.js', 'w', encoding='utf-8') as f:
            f.write(sync_code)
        print("✓ Repaired sync.js payload mapping.")

    # ---------------------------------------------------------------------
    # 2. REPAIR APP.JS (Global Navigation Controller & Click Listener)
    # ---------------------------------------------------------------------
...
"""
            ...
            for i in range(start_idx, len(app_code)):
                if app_code[i] == '{':
                    ...
                    if brace_count == 0:
                        ...
                        break
            if end_idx != -1:
                app_code = app_code[:start_idx] + nav_controller_js.strip() + app_code[end_idx:]
        else:
            app_code += '\n' + nav_controller_js.strip() + '\n'
    ...
    # ---------------------------------------------------------------------
    # 3. REPAIR APP.CSS (Statbook Palette, Z-Index, Hide Workbook Tabs)
    # ---------------------------------------------------------------------
...
/* Hide un-styled secondary / lower-left workbook sheet tabs */
.workbook-tabs,
.sheet-tabs,
.bottom-nav-floating,
.pwa-bottom-bar,
...
"""
        with open('app.css', 'a', encoding='utf-8') as f:
            f.write(master_css)
        print("✓ Repaired app.css with Statbook theme and button colors.")

    # ---------------------------------------------------------------------
    # 4. REPAIR INDEX.HTML (Guardrails Wording & Auto Cache Flush)
    # ---------------------------------------------------------------------
...
"""
        ...
        with open('index.html', 'w', encoding='utf-8') as f:
            f.write(html)
        print("✓ Repaired index.html (Guardrails wording & auto cache flush).")
    ...
    print("\n🎉 All Momentum PWA repairs executed successfully!")
